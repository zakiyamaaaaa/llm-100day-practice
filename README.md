...
教材全体を7フェーズ・21クエストに体系化した、日本語のブラウザゲームを収録しています。各クエストの「ワークショップを開始」から、理論を読み、コードまたは設計メモを書き、ブラウザ内の検証シミュレーションを通過できます。回答・学習ノート・検証結果・完了状態はクエストごとにブラウザの`localStorage`へ自動保存され、外部チャットやAPIキーは必要ありません。
